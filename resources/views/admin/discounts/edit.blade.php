        ...
        <form action="{{ route('admin.discounts.update', $discount->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <div class="mt-2 space-y-4">
                    <div class="flex items-center">
                        <input id="discount_type_amount" name="discount_type" type="radio" value="amount" class="h-4 w-4 text-indigo-600 border-gray-300 rounded dark:border-gray-600" {{ old('discount_type', $discount->discount_amount ? 'amount' : 'percent') == 'amount' ? 'checked' : '' }}>
                        <label for="discount_type_amount" class="ml-3 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Fixed Amount (RM)
                        </label>
                    </div>
                    <div class="flex items-center">
                        <input id="discount_type_percent" name="discount_type" type="radio" value="percent" class="h-4 w-4 text-indigo-600 border-gray-300 rounded dark:border-gray-600" {{ old('discount_type', $discount->discount_percent ? 'percent' : 'amount') == 'percent' ? 'checked' : '' }}>
            ...
            <div class="mb-4">
                ...
                <div class="mt-1 flex rounded-md shadow-sm">
                    <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300" id="discount_symbol">
                        {{ $discount->discount_amount ? 'RM' : '%' }}
                    </span>
                    <input type="number" name="discount_value" id="discount_value" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" placeholder="0.00" step="0.01" min="0" value="{{ old('discount_value', $discount->discount_amount ?? $discount->discount_percent) }}" required>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="datetime-local" name="starts_at" id="starts_at" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" value="{{ old('starts_at', $discount->starts_at ? $discount->starts_at->format('Y-m-d\TH:i') : '') }}">
                    @error('starts_at')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="ends_at" id="ends_at" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" value="{{ old('ends_at', $discount->ends_at ? $discount->ends_at->format('Y-m-d\TH:i') : '') }}">
                    @error('ends_at')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="description" id="description" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" placeholder="E.g., Summer Sale, New Release Discount" value="{{ old('description', $discount->description) }}">
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <div class="flex items-center">
                    ...
                    <input id="is_active" name="is_active" type="checkbox" value="1" class="h-4 w-4 text-indigo-600 border-gray-300 rounded dark:border-gray-600" {{ old('is_active', $discount->is_active) ? 'checked' : '' }}>
