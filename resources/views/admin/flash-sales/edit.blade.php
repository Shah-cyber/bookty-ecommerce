...
<div class="container mx-auto px-4 py-8 dark:bg-gray-900 dark:text-gray-100">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Edit Flash Sale</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6 dark:bg-gray-800 dark:shadow-none">
        <form action="{{ route('admin.flash-sales.update', $flashSale->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" placeholder="e.g., Weekend Flash Sale" value="{{ old('name', $flashSale->name) }}" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="description" id="description" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" placeholder="e.g., Limited time offers on bestsellers" value="{{ old('description', $flashSale->description) }}">
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-2 space-y-4">
                            <div class="flex items-center">
                                <input id="discount_type_fixed" name="discount_type" type="radio" value="fixed" class="h-4 w-4 text-indigo-600 border-gray-300 rounded dark:border-gray-600" {{ old('discount_type', $flashSale->discount_type) == 'fixed' ? 'checked' : '' }}>
                                <label for="discount_type_fixed" class="ml-3 block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    Fixed Amount (RM)
                                </label>
                            </div>
                            <div class="flex items-center">
                                <input id="discount_type_percentage" name="discount_type" type="radio" value="percentage" class="h-4 w-4 text-indigo-600 border-gray-300 rounded dark:border-gray-600" {{ old('discount_type', $flashSale->discount_type) == 'percentage' ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm">
                            <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300" id="discount_symbol">
                                {{ $flashSale->discount_type == 'fixed' ? 'RM' : '%' }}
                            </span>
                            <input type="number" name="discount_value" id="discount_value" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" placeholder="0.00" step="0.01" min="0" value="{{ old('discount_value', $flashSale->discount_value) }}" required>
                        </div>
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="datetime-local" name="starts_at" id="starts_at" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" value="{{ old('starts_at', $flashSale->starts_at->format('Y-m-d\\TH:i')) }}" required>
                        @error('starts_at')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="ends_at" id="ends_at" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" value="{{ old('ends_at', $flashSale->ends_at->format('Y-m-d\\TH:i')) }}" required>
                        @error('ends_at')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            @foreach($books as $book)
                                <div class="book-item" data-genre="{{ $book->genre_id }}">
                                    <div class="flex items-start space-x-2">
                                        <input type="checkbox" name="books[]" id="book_{{ $book->id }}" value="{{ $book->id }}" class="h-4 w-4 text-indigo-600 border-gray-300 rounded mt-1 book-checkbox dark:border-gray-600" 
                                            {{ in_array($book->id, old('books', $flashSale->books->pluck('id')->toArray())) ? 'checked' : '' }}>
                                    ...
                                    <div class="mt-2 ml-6">
                                        ...
                                        <input type="number" name="special_prices[{{ $book->id }}]" id="special_prices_{{ $book->id }}" class="mt-1 focus:ring-indigo-500 focus;border-indigo-500 block w-full text-xs border-gray-300 rounded-md special-price-input dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" placeholder="Custom price" step="0.01" min="0" 
                                            value="{{ old('special_prices.' . $book->id, isset($specialPrices[$book->id]) ? $specialPrices[$book->id] : '') }}">
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="flex justify-end">
                <button type="submit" id="submit-btn" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded">
                    Update Flash Sale
                </button>
            </div>
            <div id="validation-error" class="text-red-500 text-sm mt-2 hidden">
                Please select at least one book for the flash sale.
            </div>
        </form>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let currentType = document.getElementById('discount_type_fixed').checked ? 'fixed' : 'percentage';
        // Remember last known values per type so toggling restores user input
        ...
        function switchType(newType) {
            // store the value under the previous type bucket
            if (currentType === 'fixed') {
                lastFixedValue = discountInput.value;
            } else {
                lastPercentageValue = discountInput.value;
            }
            // pick value for new type (restore last known or default 0)
            if (newType === 'fixed') {
                discountInput.value = lastFixedValue && !isNaN(parseFloat(lastFixedValue)) ? lastFixedValue : '0';
            } else {
                discountInput.value = lastPercentageValue && !isNaN(parseFloat(lastPercentageValue)) ? lastPercentageValue : '0';
            }
            applyTypeAttributes(newType);
            currentType = newType;
        }
        
        // Keep last-known value updated as the user types
        ...
        // Initial setup
        // Initial attributes only, do not change value
        ...
        form.addEventListener('submit', function(event) {
            ...
            const checkedBooks = Array.from(bookCheckboxes).filter(checkbox => checkbox.checked);
            
            if (checkedBooks.length === 0) {
                event.preventDefault();
                validationError.classList.remove('hidden');
                window.scrollTo({
                    top: validationError.offsetTop - 100,
                    behavior: 'smooth'
                });
            } else {
